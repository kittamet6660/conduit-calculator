...
with tab3:
    ...
    k = st.selectbox("ชนิดสายไฟ", list(WIRE_SPECS.keys()))
    inf = WIRE_SPECS[k]
    ...
    with c1:
        if os.path.exists(inf['img']):
            st.image(inf['img'], use_container_width=True) # ปรับให้เต็มคอลัมน์
        else:
            st.warning("ไม่พบรูปภาพ")

    # ส่วนแสดงข้อมูล (จัดรูปแบบใหม่)
    with c2:
        st.subheader(k) # แสดงชื่อหัวข้อใหญ่ๆ
        st.info(f"📌 {inf['desc']}") # ใส่กรอบสีฟ้าให้คำอธิบาย
        
        st.markdown("### ✅ ข้อกำหนดการใช้งาน:")
        # วนลูปเพื่อแสดงรายการทีละบรรทัด ไม่ให้เป็น JSON
