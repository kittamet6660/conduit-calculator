...
DATA_FILE = "iec_data_v8.json"  # เปลี่ยนชื่อไฟล์เป็น v8
...
# --- ข้อมูล OD สายไฟ (คงเดิมตามที่คุณให้มา) ---
DEFAULT_DATA = {
    "IEC 01 (THW)": [
...
# --- [UPDATE] ฟังก์ชันคำนวณท่อ (กลับไปใช้ 40% คงที่) ---
def get_conduit_fill_limit(qty):
    return 0.40  # Flat rate 40% สำหรับทุกกรณี

def calc_conduit_max_wires(wire_od, conduit_id):
    wire_area = math.pi * ((wire_od / 2) ** 2)
    conduit_area = math.pi * ((conduit_id / 2) ** 2)
    # คำนวณที่ 40% อย่างเดียว
    return math.floor((conduit_area * 0.40) / wire_area)

# ฟังก์ชันคำนวณ Wire Way (คงไว้ที่ 20%)
def calc_wireway_max_wires(wire_od, wireway_area):
    wire_area = math.pi * ((wire_od / 2) ** 2)
    usable_area = wireway_area * 0.20 
...
st.caption("มาตรฐาน วสท./IEC | ท่อร้อยสาย (Fill 40%) | รางเดินสาย (Fill 20%) | By : Gonnatee & Krittamet (EEG453 | ดร.สำเริง ฮินท่าไม้)")
...
# ==========================================
# Tab 1: ท่อร้อยสาย (40% All)
# ==========================================
with tab1:
    st.header("⚪ คำนวณท่อร้อยสาย (Conduit)")
    st.caption("เกณฑ์การคำนวณ: พื้นที่หน้าตัดสายรวมไม่เกิน 40% ของพื้นที่หน้าตัดท่อ")
    ...
    if calc_mode == "🅰️ หาขนาดท่อ (ใส่จำนวนเส้น)":
        ...
        if st.button("🚀 คำนวณขนาดท่อ", type="primary", key="btn_c_calc"):
            ...
            fill_limit = get_conduit_fill_limit(qty) # ได้ค่า 0.40 เสมอ
        ...
        if st.button("🔢 คำนวณจำนวนเส้น", type="primary", key="btn_c_max"):
            ...
            st.caption(f"(คำนวณที่ Fill Factor 40% ในท่อ {selected_conduit_size})")

# ==========================================
# Tab 2: รางเดินสาย (Wire Way)
# ==========================================
with tab2:
