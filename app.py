...
# --- ฟังก์ชันสร้าง PDF (Professional) ---
def create_pdf(wires_list, total_area, result_name, percent_fill, mode_name, logo_upload=None, inspector_name="", inspector_pos=""):
    ...
    if logo_upload is not None:
        try:
            ...
            # Center the logo (A4 width = 210mm)
            logo_w = 40
            x_pos = (210 - logo_w) / 2
            pdf.image("temp_logo.png", x=x_pos, y=10, w=logo_w)
            pdf.ln(35) # Move cursor down
        except:
            pass
    else:
        pdf.ln(10)
    ...
    for w_type in unique_types:
        ...
        if spec and os.path.exists(spec['img']):
            ...
            x_start += 35 # Space between images
    
    pdf.set_y(y_start + 45) # Move below images
    ...
    pdf.rect(10, pdf.get_y(), 190, 35, 'F') # Green Box
