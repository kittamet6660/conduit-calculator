...
def get_thai_time():
    # เวลาปัจจุบัน (Server อาจเป็น UTC) + 7 ชั่วโมง
...
def create_pdf(wires_list, total_area, result_name, percent_fill, mode_name, logo_upload=None, inspector_name="", inspector_pos="", custom_date_str=""):
    ...
    # --- ใช้วันที่จาก Custom String หรือ TimeNow+7 ---
    pdf.set_font('Thai' if has_font else 'Arial', '', 14)
    
    if custom_date_str:
        date_to_show = custom_date_str
    else:
        # Fallback: ใช้เวลาปัจจุบัน + 7 ชั่วโมง
...
# --- คำนวณเวลาไทยสำหรับ Default Value ---
...
with st.sidebar:
    ...
    # --- ส่วนเลือกวันที่ (ค่าเริ่มต้นเป็นเวลาไทย) ---
    st.write("---")
    st.header("📅 วันที่ในเอกสาร")
    user_date = st.date_input("เลือกวันที่", value=bkk_now) # ใช้ bkk_now
    user_time = st.time_input("เลือกเวลา", value=bkk_now) # ใช้ bkk_now
    
    # รวมวันที่และเวลาเป็น String
    formatted_date_str = datetime.combine(user_date, user_time).strftime("%d/%m/%Y %H:%M")
