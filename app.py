...
# --- ฟังก์ชันสร้าง PDF (UPDATED: โลโก้ตรงกลาง) ---
def create_pdf(wires_list, total_area, result_name, percent_fill, mode_name, logo_upload=None, inspector_name="", inspector_pos=""):
    ...
    current_y = 15 # ระยะห่างขอบบนมาตรฐาน (ถ้าไม่มีโลโก้)
    ...
    if logo_upload is not None:
        try:
            ...
            # คำนวณตำแหน่ง X เพื่อให้ภาพอยู่ "กึ่งกลางกระดาษ"
            # กระดาษ A4 กว้าง ~210 mm
            page_width = 210
            center_x = (page_width - final_w) / 2
            
            # วาดรูปที่กึ่งกลาง ด้านบน (เว้นขอบบน 10mm)
            pdf.image(tmp_path, x=center_x, y=10, w=final_w, h=final_h)
            
            # อัปเดตตำแหน่ง Y ให้ขยับลงมาใต้โลโก้
    ...
    # 2. Header & Date
    # ตั้งค่าตำแหน่ง Y ตามที่คำนวณไว้ (ใต้โลโก้ หรือ ที่เดิมถ้าไม่มีโลโก้)
    pdf.set_y(current_y)
    
    pdf.set_font('Thai' if has_font else 'Arial', '', 24)
    # ใช้ cell(0) เพื่อให้กว้างเต็มหน้า และ align='C' เพื่อจัดกลาง
